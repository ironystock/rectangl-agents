...
#### Missing Ownership Verification (CRITICAL)
For endpoints that accept resource IDs from user input:
```bash
# Find handlers that accept resource_id, prompt_id, collection_id without ownership checks
grep -rn --include="*.go" 'resource_id\|prompt_id\|collection_id' internal/server/ | grep -v '_test.go'
```
- **Review**: Each endpoint accepting resource IDs must verify ownership
- **Pass**: Handler checks resource exists in user's database before processing
- **Pattern required**:
```go
// Verify user owns this resource
userDB, err := s.db.GetUserDB(userID)
if err == nil {
    var exists bool
    userDB.QueryRow("SELECT EXISTS(SELECT 1 FROM table WHERE id = ?)", resourceID).Scan(&exists)
    if !exists {
...
```
- **Fail**: Resource ID from input used without ownership verification

### 6b. Input Allowlist Validation

#### Missing Allowlist for Enumerated Values
When accepting enumerated/categorical values from user input:
```bash
# Find handlers accepting type, format, platform, status parameters
grep -rn --include="*.go" 'req\.\(Type\|Format\|Platform\|Status\|ResourceType\)' internal/server/ | grep -v '_test.go'
```
- **Review**: All categorical inputs must be validated against allowlist
- **Pass**: Uses map[string]bool or switch/case validation before processing
- **Pattern required**:
```go
validValues := map[string]bool{"option1": true, "option2": true}
if !validValues[req.Type] {
    c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid type"})
    return
}
```
- **Fail**: Categorical input used without allowlist validation
- **Risk**: Log injection, analytics corruption, potential XSS if displayed

### 6c. Partial Update Data Loss

#### UPDATE Statements That Overwrite Unrelated Columns
```bash
# Find multi-column UPDATEs in handlers that parse request fields
grep -rn --include="*.go" 'UPDATE.*SET.*=.*,.*=.*,.*=' internal/server/ | grep -v '_test.go\|is_deleted'
```
- **Review**: When handler accepts optional fields, verify they don't overwrite unrelated columns with empty values
- **Pass**: Either (a) all SET columns validated/required, or (b) handler uses conditional field updates (`_field` param, dynamic SET, pointer nil checks)
- **Fail**: UPDATE with 3+ columns where some may be zero-value from partial request
- **Risk**: Silent data loss — e.g., editing title wipes model_id, description, linked fields
