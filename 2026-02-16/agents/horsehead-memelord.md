...
### Lessons Learned: CSP Violation Incident (2026-01)

**What happened:** Inline event handlers were introduced in several templ files:
- `sidebar.templ` - `onkeydown` for ESC key on collection form
- `prompts/detail.templ` - `onclick` for tab switching
- `prompts/versions.templ` - `onclick` for modal close
- `models/model_picker.templ` - OOB script swap without nonce

**Why it happened:**
1. Quick prototyping without consulting the CSP rules
2. Copy-pasting patterns from other codebases that allow inline handlers
3. Not running the pre-commit checklist before merging
4. Assuming "simple" JS is okay inline (it never is with our CSP)

**How it was fixed:**
1. Created `tabs.js` for CSP-compliant tab switching via `data-action="switch-tab"`
2. Created `modals.js` for CSP-compliant modal close via `data-action="close-modal"`
3. Extended `form-escape.js` to handle `data-escape-cancel` for inline forms
4. Replaced OOB script swap with OOB element swap + `htmx:oobAfterSwap` event

**Prevention:**
- ALWAYS use `data-action` attributes for click/keyboard handlers
- ALWAYS use external JS files loaded in `app_base.templ`
- NEVER add inline `onclick`, `onkeydown`, etc. to templ files
- NEVER use OOB script swaps - use OOB element swaps with event listeners
- Run the pre-commit checklist (search for `on[a-z]+=` in templ files)
