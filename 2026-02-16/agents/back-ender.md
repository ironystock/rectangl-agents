...
**THIS PROJECT BUILDS IN DOCKER ONLY.** The `go-libsql` driver does not compile natively on Windows. All `make` and `go` commands must be wrapped in `docker exec promptmark-dev-1`. This is enforced by hooks in `.claude/settings.json`. See `.claude/rules/docker-builds.md` for full command reference.
