...
## Token Density Check (Chore Branches Only)

> **When to run**: Only during `chore/*` branches, not every PR
> **Purpose**: Identify files that tokenize poorly (e.g., SVG paths, base64 data)

### High Token-to-Line Ratio Detection

Some files have deceptively low line counts but consume massive tokens:
- SVG icon files (~60 tokens/line)
- Base64 embedded assets
- Minified or dense data

```bash
# Check for high-density files (rough heuristic using file size / lines)
for f in $(find cmd/web -name "*.templ" -size +20k); do
    lines=$(wc -l < "$f")
    bytes=$(stat -f%z "$f" 2>/dev/null || stat -c%s "$f")
    ratio=$((bytes / lines))
    if [ "$ratio" -gt 100 ]; then
        echo "HIGH DENSITY: $f (${bytes} bytes, ${lines} lines, ~${ratio} bytes/line)"
    fi
done
```

| Bytes/Line | Tokens/Line (est.) | Status | Action |
|------------|-------------------|--------|--------|
| <50 | ~10-15 | Normal | No action |
| 50-100 | ~15-40 | Warning | Monitor |
| >100 | ~40-80 | Critical | Split file |

### Mitigation Strategies

1. **SVG Icons**: Split into category files (navigation, social, status, etc.)
2. **Embedded Assets**: Move to external files, load at runtime
3. **Dense Data**: Extract to JSON/config files

### Example: Icon File Split

Before: `icons.templ` (624 lines, 37K tokens, 19% of context)
After: 12 category files (each 2-6K tokens, ~1-3% of context each)

```
icons_navigation.templ  - Dashboard, Home, Menu
icons_documents.templ   - File, Folder, Paper
icons_actions.templ     - Write, Search, Plus
icons_status.templ      - Check, Warning, Love
icons_social.templ      - X, GitHub, Discord
icons_helpers.templ     - SocialPlatformIcon(), etc.
```
