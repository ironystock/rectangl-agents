...
### 7. Hugo Documentation Site

#### Hugo Build Check
```bash
cd docs/site && hugo --renderToMemory 2>&1
```
- **Pass**: Hugo builds without errors
- **Warn**: Build errors in documentation site

#### MCP Tool Parity
Check that all MCP tools registered in `internal/mcp/tools.go` are documented in `docs/site/content/api/mcp-reference.md`:
```bash
# Extract tool names from tools.go
grep 'Name:' internal/mcp/tools.go | grep -oE '"[^"]*"' | tr -d '"' | sort > /tmp/mcp-tools.txt
# Extract tool names from docs
grep 'mcp-tool name=' docs/site/content/api/mcp-reference.md | grep -oE '"[^"]*"' | tr -d '"' | sort > /tmp/mcp-docs.txt
diff /tmp/mcp-tools.txt /tmp/mcp-docs.txt
```
- **Pass**: All tools documented
- **Warn**: Tools missing from documentation

### 8. Swagger Annotations
When new REST endpoints are added, check for swag annotations:
```bash
git diff --cached internal/server/*.go | grep "func.*Handler" | while read line; do
    # Check if swag annotation exists above the function
    echo "Check for @Summary or @Router annotation"
done
```
- **Pass**: New handlers have swag annotations
- **Warn**: Missing swagger documentation for new endpoints
...
6. **Hugo site**: Does it build? Are new features documented?
7. **MCP tools**: Are all tools in the reference?
8. **Swagger**: Do new REST endpoints have annotations?
